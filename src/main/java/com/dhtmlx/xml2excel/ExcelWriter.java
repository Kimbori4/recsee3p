...
public class ExcelWriter extends BaseWriter {
	...
	public void generate(String xml, HttpServletResponse resp, HttpServletRequest req){
	...
	private void outputExcel(HttpServletResponse resp, HttpServletRequest req) throws IOException, WriteException {

		String fileName = "counsel_manage_report_";
		if(req.getParameter("fileName") != null) fileName = req.getParameter("fileName") + "_";

		Date date = new Date();
		resp.setContentType("application/vnd.ms-excel");
		resp.setCharacterEncoding("UTF-8");
		resp.setHeader("Content-Disposition", "attachment;filename="+fileName+DateUtil.toString(date)+".xls");
		...
		for (int i = 0; i < rows.length; i++) {
			...
			for (int j = 0; j < cells.length; j++) {
				...
				try {
					...
					if("합계".equals(name.trim())&&j==count+1) {
						name="";
						Label label = new Label(j, i + headerOffset, "", f);
						sheet.addCell(label);
						count=j;
					}else {
						Label label = new Label(j, i + headerOffset, name, f);
