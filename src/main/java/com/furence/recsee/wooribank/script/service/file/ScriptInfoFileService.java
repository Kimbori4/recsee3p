...
import com.furence.recsee.wooribank.script.param.request.FileDownloadParam.ScriptCurrentInfo;
...
@Service("scriptInfoFileService")
public class ScriptInfoFileService extends FileService<ScriptInfo, FileDownloadParam.ScriptCurrentInfo>{

	private static final Logger logger = LoggerFactory.getLogger(ScriptInfoFileService.class);
	
	private final int TOTAL_COLUMNS = 3;
	
	@Autowired
	public ScriptInfoFileService(FileContentDao fileContentDao) {
		super(fileContentDao);
	}
	
	@Override
	public FileServiceType getType() {
		return FileServiceType.ScriptInfo;
	}
	 ...
	 * @return
	 */
	private ScriptInfo getProductCurrentScriptInfo(ScriptCurrentInfo param) throws Exception {		
		return ScriptInfo.from(this.fileContentDao.selectProductScriptInfo(param));
	}
	
	@Override
	protected ScriptInfo getFileContent(ScriptCurrentInfo param) {
		ScriptInfo info = null;
		
		try {
			info = this.getProductCurrentScriptInfo(param);
		} catch (Exception e) {
	...
	@Override
	protected File makePDF(ScriptInfo info, ScriptCurrentInfo param)  {
	 ...
	 */
	@Override
	protected File makeExcel(ScriptInfo info, ScriptCurrentInfo param) {
