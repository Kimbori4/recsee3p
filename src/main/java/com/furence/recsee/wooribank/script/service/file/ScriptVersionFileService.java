...
import com.furence.recsee.wooribank.script.param.request.FileDownloadParam.VersionSnapshot;
...
@Service("versionFileService")
public class ScriptVersionFileService extends FileService<ScriptSnapshot, FileDownloadParam.VersionSnapshot> {
	
	private static final Logger logger = LoggerFactory.getLogger(ScriptVersionFileService.class);
	
	private final int TOTAL_COLUMNS = 3;
		
	@Autowired
	public ScriptVersionFileService(FileContentDao fileContentDao) {
		super(fileContentDao);
	}
	
	@Override
	public FileServiceType getType() {
		return FileServiceType.History;
	}
	 ...
	 * @return
	 */
	private ScriptSnapshot getScriptSnapShot(VersionSnapshot param) {		
		return this.fileContentDao.selectScriptVersionSnapshot(param);
	}
	
	/**
	 * 버전에 해당하는 스크립트 스냅샷 json 데이터 가져오기
	 * @param history
	 * @return
	 */
	@Override
	protected ScriptSnapshot getFileContent(VersionSnapshot param) {	
		return this.getScriptSnapShot(param);
	}
	...
	@Override
	protected File makePDF(ScriptSnapshot snapshot, VersionSnapshot param)  {
		...
		try{
			...
			String file = this.filePath 
					+ snapshot.getScriptInfo().getProductName() 
					+ "_" + this.sdf.format(snapshot.getUpdateDate()) 
					+ "_" + param.getScriptVersion()
					+ ".pdf";
			...
			// 상품명
			String productName = Optional.ofNullable(snapshot.getScriptInfo().getProductName()).orElse("");
			
			// 상품코드
			String productCode = Optional.ofNullable(snapshot.getScriptInfo().getProductCode()).orElse("");
			...
			List<Step> stepList = snapshot.getScriptInfo().getStepList();
			int index = 0;
	 ...
	 */
	@Override
	protected File makeExcel(ScriptSnapshot snapshot, VersionSnapshot param) {
		...
		productCodePos = 0;
		ExcelMaker.setValue(productCode, "상품코드: " + snapshot.getScriptInfo().getProductCode(), productCodePos++);
		...
		productNamePos = 0;
		ExcelMaker.setValue(productName, "상품명: " + snapshot.getScriptInfo().getProductName(), productNamePos++);
		...
		modelMap.put("target", snapshot.getScriptInfo().getProductName() 
				+ "_" + this.sdf.format(snapshot.getUpdateDate()) 
				+ "_" + param.getScriptVersion());
