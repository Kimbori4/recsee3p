 ...
 * LSH512 알고리즘 구현
 * 
 * 워드 길이: 64-bit (8-byte) 연쇄 변수 길이: 1024-bit (128-byte) 메시지 블록 길이: 2048-bit
 * (256-byte)
 */
public class LSH512 extends Hash {
	...
	public void update(byte[] data, int offset, int lenbits) {
		...
		if (blkidx > 0 && rbytes >= gap) {
			System.arraycopy(data, offset, block, blkidx, gap);
			compress(block, 0);
			boff = 0;
			rbytes -= gap;
			offset += gap;
		...
		if (rbytes > 0) {
			blkidx = boff >> 3;
			System.arraycopy(data, offset, block, blkidx, rbytes);
			boff += rbytes << 3;
			offset += rbytes;
		}
