			...
			rc.updateWave = function(path, isTx) {
				...
				requestWave(function (data){
					var heightArray = data||rc.currentWave;
					 ...
					 $body.find(".stt_side_view, .stt_center_view, .stt_right_view").height(height*5 + 50)
		...
		rc.sttParserInsert = function( rx , tx ){
			...
			for(var i = 0 ; i < len ; i++){
				...
				if(rxTag[i] != undefined){
					...
					$(".stt_rx_view").append("<div class='stt_rx_text_wrap stt_text_wrap' style='margin-top:"+rxb * 50+"px;' begin='"+rxb+"' end='"+rxe+"'>"
				...
				if(txTag[i] != undefined){
					...
					$(".stt_tx_view").append("<div class='stt_tx_text_wrap stt_text_wrap' style='margin-top:"+txb * 50+"px;'  begin='"+txb+"' end='"+txe+"'>"
			...
			TempArr.forEach(function(val , idx ){
				...
				if(flag == "RX"){
					sttAllText.append("<span class='rx_t'>고객<font begin="+begin+">["+seconds2time(begin)+"]</font></span>");
					sttAllText.append(val);
				}else{
					sttAllText.append("<span class='tx_t'>상담사<font begin="+begin+">["+seconds2time(begin)+"]</font></span>");
					sttAllText.append(val);
				}
			})
...
/*
	        ...
	        $player.find(".stt_play_map").click(function(event){
	        	...
	        	// 구간이동~~
	        	var choose = (event.clientY - 110) / all;					//	 offsetY 였으나 구간이 늘어날 시 offsetY 쓰면 안되어서 ClientY 로 교체
