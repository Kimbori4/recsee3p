...
var time=0;
var checkEnd;
var recsee_player_ui_html = "face_recsee_player_ko.ui.html";
"use strict";
// 초기 CSS 부르기
...
(function(src) {
//	console.log("src : " + src)
	src = "/recsee3p/resources/component/recsee_player/recsee_player.js?ver=1234";
    ...
    $("head").append('<link rel="stylesheet" type="text/css" href="' + path + '/faceRecsee_player.css">');
...
RecseePlayer.prototype.setDefault = function() {
    ...
    */
			...
			'<div class="popup_header">'+
				'<div class="ui_pannel_popup_header">'+
					'<div class="ui_float_left">'+
						'<p class="ui_pannel_tit">플레이어 태그</p>'+
					'</div>'+
    ...
    '<div class="popup_obj sectionSetting" style="width:168px; height: 124px !important;">'+
	    '<div class="ui_popup_padding">'+
	        '<div class="popup_header">'+
	            '<div class="ui_pannel_popup_header">'+
	                '<div class="ui_float_left">'+
...
//        		str += top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text8+" : "/*"내선번호 : "*/+ext
//    		if(userName != null && userName != undefined && userName != "")
//        		str += top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text6+" : "/*" 상담원이름 : "*/+userName
//    		if(PhoneNumber != null && PhoneNumber != undefined && PhoneNumber != "")
//        		str += top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text7+" : "/*" 고객 번호 : "*/+PhoneNumber
//        	if(str.length > 0)
//        		str = top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text16+" : "/*"청취중 정보 - "*/ + str
        	if(ext != null && ext != undefined && ext != "")
        		str += "내선번호 : "/*"내선번호 : "*/+ext
    		if(userName != null && userName != undefined && userName != "")
        		str += "상담사명 : "/*" 상담원이름 : "*/+userName
    		if(PhoneNumber != null && PhoneNumber != undefined && PhoneNumber != "")
        		str += "고객번호 : "/*" 고객 번호 : "*/+PhoneNumber
        	if(str.length > 0)
        		str = "청취중 정보 -  : "/*"청취중 정보 - "*/ + str
        ...
        if(rc.type[TYPES.AUDIO]) {
            ...
            // 파일 설정(로드) 이벤트
            $audio.setFile = function(file, name, uploadFile, all, wt, v) {
            	if (v == undefined) {
            		v = "";
            	}
            	if(uploadFile){
            	...
            	if(!rc.type[TYPES.DUAL]){
            		$audio.attr("src", file+"&recVolume="+v);
            	}
            	else
            		$audio.attr("src", file.replace('all','rx')+"&recVolume="+v);
    			...
    			setTimeout(function(){
                	if(rc.type[TYPES.WAVE]){
                    	if(!rc.type[TYPES.DUAL]){
                			...
                			rc.updateWave(getParameterFromUrl("url", rc.requestParam.replace('all','rx')),false,all)
                		}                    		
                	}
                },1000)        		
             ...
             * */
            // 파일 설정(로드) 이벤트
            $tx_audio.setFile = function(file, name, uploadFile, all, v) {
            	if (v == undefined) {
            		v = "";
            	}
            	if(uploadFile){
            	...
            	$tx_audio.attr("src", file.replace('all','tx')+"&recVolume="+v);
                ...
                if(rc.type[TYPES.WAVE]){
            		rc.initWave();
            		rc.updateWave(getParameterFromUrl("url", rc.requestParam.replace('all','tx')),true,all)
            	}
            }
        ...
        * @type function
        * @description RecseePlayer 객체에 파일을 설정한다
        * @param t(string): type[audio, video], f(file), u(upload), wt(waveType), v(recVolume)
        */
        rc.setFile = function(t, f, u, a, wt, v) {
        	if (v == undefined) {
        		v = "";
        	}
        	rc.all = a||false;
        	...
        	if(t == "audio" || t == "video") {
        		...
        		if(t == "audio" && rc.type[TYPES.AUDIO]) {
        			$audio.setFile(file, name, u, a, wt, v);
        			if(rc.type[TYPES.DUAL])
        				$tx_audio.setFile(file, name, u, a, v);
        ...
        */
        ...
        rc.addPlayList = function(recFileData,str){
        	...
        	var recVolume = recFileData.recVolume;

        	var listenUrl = recFileData.recDate+recFileData.recTime.replace(/:/gi,"")+recFileData.recExt;

        	var htmlString = "<li listenUrl='"+listenUrl+"' recDate='"+recDate+"' recTime='"+recTime+"' recExt='"+recExt+"' recVolume='"+recVolume+"'><input type='checkbox' class='listCheckBox' style='display:none'/><span>";
        	if(top.telnoUse!='Y'){
//        		htmlString += "<b>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text4/*일자*/+"</b> : ";
        		htmlString += "<b>"+"일자"/*일자*/+"</b> : ";
        	}
        	htmlString += recDate+"</span><span>" +
//        	"<b>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text5/*시간*/+"</b> : " +recTime+"</span>";
        	"<b>"+"시간"/*시간*/+"</b> : " +recTime+"</span>";
        	if(top.telnoUse!='Y'){
        		htmlString += "<span><b>"+"상담사명"/*상담사명*/+"</b> : "+recUserName+"</span>";
        	}
        	
        	htmlString += "<span><b>"+"고객번호"/*고객번호*/+"</b> : "+recCustPhone+"</span>";
        	if(top.telnoUse!='Y'){
        		htmlString += "<span><b>"+"내선번호"/*내선번호*/+"</b> : "+recExt+"</span>";
        	}
        	htmlString += "</li>";
...
//        		alert(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg40/*"이미 플레이 리스트에 추가된 녹취파일입니다."*/)
        		alert("이미 플레이 리스트에 추가된 녹취파일입니다."/*"이미 플레이 리스트에 추가된 녹취파일입니다."*/)
        	}
        }
...
//    					alert(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg26/*"요청에 실패 하였습니다."*/);
    					alert("요청에 실패 하였습니다."/*"요청에 실패 하였습니다."*/);
    				}
    			}
    		});
...
//        		htmlString += "<li class='memoMarking'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text9+"</li>"
        		htmlString += "<li class='memoMarking'>"+"플레이어 태그"+"</li>"
...
//	        	htmlString += "<li class='playMarkingLoop'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text17/*반복*/+"</li>"
	        	htmlString += "<li class='playMarkingLoop'>"+"반복"/*반복*/+"</li>"
//	        	htmlString += "<li class='playMarkingLoopDel' style='display:none;'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text18/*반복해제*/+"</li>"
	        	htmlString += "<li class='playMarkingLoopDel' style='display:none;'>"+"반복해제"/*반복해제*/+"</li>"
        	}
        	// 다운 사용 할 때만 사용
...
//        		htmlString += "<li class='downMarking'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text19/*다운*/+"</li>"
        		htmlString += "<li class='downMarking'>"+"다운"/*다운*/+"</li>"
        	// 구간 묵음 사용 할 때만 사용
...
//        		htmlString += "<li class='muteMarking'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text20/*묵음*/+"</li>"
        		htmlString += "<li class='muteMarking'>"+"묵음"/*묵음*/+"</li>"
//        		htmlString += "<li class='muteMarkingDel' style='display:none;'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text21/*묵음해제*/+"</li>"
        		htmlString += "<li class='muteMarkingDel' style='display:none;'>"+"묵음해제"/*묵음해제*/+"</li>"
        	}
        	// 구간 제거 사용 할 때만 사용
...
//        		htmlString += "<li class='cutoffMarking'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text22/*제거*/+"</li>"
        		htmlString += "<li class='cutoffMarking'>"+"제거"/*제거*/+"</li>"
//        		htmlString += "<li class='cutoffMarkingDel' style='display:none;'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text23/*제거해제*/+"</li>"
        		htmlString += "<li class='cutoffMarkingDel' style='display:none;'>"+"제거해제"/*제거해제*/+"</li>"
        	}
...
//                	htmlString += "<li class='deleteMarking'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text24/*삭제*/+"</li>"
                	htmlString += "<li class='deleteMarking'>"+"삭제"/*삭제*/+"</li>"
                	htmlString += "</ul>";
            	}
        	}else{
//        		htmlString += "<li class='deleteMarking'>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text24/*삭제*/+"</li>"
...
//                		alert(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg41/*"다운로드 권한이 없습니다."*/);
	            	...
	            	var dataStr = {
	            			"cStartTime" : Math.round(startTime)
	            		, 	"cEndTime" : Math.round(endTime)
...
//                $('.play_info span').text(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg47/*"요청하신 녹취 파일을 찾을 수 없습니다."*/);
            	$('.play_info span').text("요청하신 녹취 파일을 찾을 수 없습니다.");
                $('.play_file_tit').text('');
...
//                if($('.play_info span').text()!=top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg47/*"요청하신 녹취 파일을 찾을 수 없습니다."*/){
            	if($('.play_info span').text()!="요청하신 녹취 파일을 찾을 수 없습니다."/*"요청하신 녹취 파일을 찾을 수 없습니다."*/){
                    ...
                    $('.play_info span').text(top.lang.views.player.html.text2/*"현재 재생 중인 파일 : "*/)
                    var infoText = $player.find(".play_file_tit").text();
//                    var infoString = (rc.recFileData && rc.recFileData.recUserName ? top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text6+" : "/*"  상담사 : "*/ + rc.recFileData.recUserName : "")
...
//                        infoString += (rc.recFileData && rc.recFileData.recShare ? " "+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text28+" : "/*" 공유 제목 : "*/ + rc.recFileData.recShare : "")
                    var infoString = (rc.recFileData && rc.recFileData.recUserName ? "상담사"+" : "/*"  상담사 : "*/ + rc.recFileData.recUserName : "")
                        infoString += (rc.recFileData && rc.recFileData.recCustName ? " / "+"고객명"+" : "/*" / 고객명 : "*/ + rc.recFileData.recCustName : "")
                        infoString += (rc.recFileData && rc.recFileData.recExt ? " / "+"내선번호"+" : "/*" / 내선번호 : "*/ + rc.recFileData.recExt : "")
                        infoString += (rc.recFileData && rc.recFileData.recCustPhone ? " / "+"고객전화번호"+" : "/*" / 고객전화번호 : "*/ + rc.recFileData.recCustPhone : "")
                        infoString += (rc.recFileData && rc.recFileData.recTime ? " / "+"통화시작시간"+" : "/*" / 통화시작시간 : "*/ + rc.recFileData.recTime : "")
                        infoString += (rc.recFileData && rc.recFileData.recShare ? " "+"공유제목"+" : "/*" 공유 제목 : "*/ + rc.recFileData.recShare : "")
...
//                        var nowPlayBack = getCookie("playBack") || target.playbackRate || 1;
                    		var nowPlayBack = parent.$("input[name='speed']:checked").val()
//                        $("input:radio[name='speed']:radio[value='"+nowPlayBack+"']").prop("checked",true)
...
//            	infoString += (rc.recFileData && rc.recFileData.recTime ? " / "+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text27+" : "/*" / 통화시작시간 : "*/ + rc.recFileData.recTime : "")
//            	infoString += (rc.recFileData && rc.recFileData.recShare ? " "+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text28+" : "/*" 공유 제목 : "*/ + rc.recFileData.recShare : "")
            	infoString = (rc.recFileData && rc.recFileData.recUserName ? "상담사"+" : "/*"  상담사 : "*/ + rc.recFileData.recUserName : "")
            	infoString += (rc.recFileData && rc.recFileData.recCustName ? " / "+"고객명"+" : "/*" / 고객명 : "*/ + rc.recFileData.recCustName : "")
            	infoString += (rc.recFileData && rc.recFileData.recExt ? " / "+"내선번호"+" : "/*" / 내선번호 : "*/ + rc.recFileData.recExt : "")
            	infoString += (rc.recFileData && rc.recFileData.recCustPhone ? " / "+"고객전화번호"+" : "/*" / 고객전화번호 : "*/ + rc.recFileData.recCustPhone : "")
            	infoString += (rc.recFileData && rc.recFileData.recTime ? " / "+"통화시작시간"+" : "/*" / 통화시작시간 : "*/ + rc.recFileData.recTime : "")
            	infoString += (rc.recFileData && rc.recFileData.recShare ? " "+"공유 제목"+" : "/*" 공유 제목 : "*/ + rc.recFileData.recShare : "")
...
//        					if((!rc.type.dual)&&confirm(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg48/*"재생중이던 파일 입니다. 이어서 재생 하시겠습니까?"*/)){
    						if((!rc.type.dual)&&confirm("재생중이던 파일 입니다. 이어서 재생 하시겠습니까?")){
...
//                    					alert(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg26/*"요청에 실패 하였습니다."*/);
            ...
            // 고객음소거
            $player.find(".play_1 button").on("click",function(event) {
            	$volume.changeValue(0).moveTo(0);
            });
            // 텔러음소거
            $player.find(".play_2 button").on("click",function(event) {
            	$tx_volume.changeValue(0).moveTo(0);
            });
...
//                    if($('.play_info span').text().indexOf(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg47)>-1){
            		if($('.play_info span').text().indexOf("요청하신 녹취 파일을 찾을 수 없습니다.")>-1){
                        return false;
                    }
...
//            		alert(top.$("#pageFrame")[0].contentWindow.lang.common.alert.noPlayerTags);            		
...
//            	if(confirm(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg55/*"현재 파일에 저장된 플레이어 태그가 모두 삭제 됩니다.\n정말 삭제 하시겠습니까?"*/)){
        		if(confirm("현재 파일에 저장된 플레이어 태그가 모두 삭제 됩니다.\n정말 삭제 하시겠습니까?")){
...
//            		alert(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg68/*"재생 중인 파일이 없습니다."*/);
            	...
            	if(top.downloadFormat=='Y'){
                	...
                	if(telnoUse=='Y'){
	            		top.$("#encAgentName").hide();
	            		top.$("#encExt").hide();
	            		top.$(".enc_ui_hide").eq(0).hide();
	            		top.$(".enc_ui_hide").eq(2).hide();	
	            		top.$("#approveUserGroup option:eq(0)").replaceWith("<option value='encY'>有</option>");
	            		top.$("#approveUserGroup option:eq(1)").replaceWith("<option value='encN'>無</option>");		
                	}
            ...
            //렉시플레이어 속도조절기능 
//            parent.$("input[name='speed']").unbind("change").bind("change",function(){
//            	var rate = parent.$("input[name='speed']:checked").val();
//            	try{
//            		audio.currentTime = audio.currentTime
//            		tx_audio.currentTime = tx_audio.currentTime
//            		audio.playbackRate = rate;
...
//            			if(confirm(top.$("#pageFrame")[0].contentWindow.lang.views.search.alert.msg69/*"체크하신 리스트에 재생중인 리스트가 포함 되어있습니다.\n재생을 중지하시고 삭제 하시겠습니까?"*/)){
	...
	if(top.unqLang=="jp") recsee_player_ui_html = "face_recsee_player_ko.ui.html";
    
    // 각 HTML 코드 작성해두는곳
    this.HTML = (function() {
        var html, wave, player;
        $.ajax({
            url: baseSrc + recsee_player_ui_html,
            dataType: "html",
            async: false,
            cache: false,
            success: function(res) {
...
//	console.log()
	...
	setCookie4("playBack",rate,14);
}

function setCookie4(cName, cValue, cDay){
	var expire = new Date();
	expire.setDate(expire.getDate() + cDay);
	var cookies = cName + "=" + escape(cValue) + "; path =/ ";
	if (typeof cDay != undefined){
