...
var time=0;
var checkEnd;
var recsee_player_ui_html = "recsee_player_ko.ui.html";
"use strict";
// 초기 CSS 부르기
    ...
    this.setEvent = function(e) {
        ...
        $audio.init = function() {
            ...
            if(rc.type[TYPES.WAVE]) {
            	...
            	console.log(rc.type[TYPES.WAVE])
                ...
                if (rc.type[TYPES.DUAL]){
                	console.log('TTTTTTTTTTTTTTTTTTTT1')
                	for(var i=0; i<BAR_COUNT; i++) {
                		var $waves = $(rc.HTML.wave);
                        var pos = parseFloat((i / BAR_COUNT * 100), 10);
                        //console.timelog(pos);
                    ...
                    $tx_wave.find(".waveObj .obj_key > div").click(function(event) {
                        ...
                        var val = $(this).data("pos");
                        $controlBar.moveTo(val);
                        top.selectNowPlayRow(tx_audio.currentTime);
                        //console.log(val)
                    });
        ...
        if(rc.type[TYPES.AUDIO]) {
            ...
            setMediaEvent(audio);
            console.log('TTTTTTTTTTTTTTTTTT888')
            // 필요한 이벤트만 받아서 처리 한다..
            if(rc.type[TYPES.DUAL]){
            	// 오디오 데이터 로드 완료 이벤트
            ...
            $audio.setFile = function(file, name, uploadFile, all, wt, v) {
            	...
            	if(!rc.type[TYPES.DUAL]){
            		...
            		$audio.attr("src", file.replace('all','tx')+"&recVolume="+v);
    			...
    			setTimeout(function(){
                	if(rc.type[TYPES.WAVE]){
                    	if(!rc.type[TYPES.DUAL]){
                			...
                			rc.updateWave(getParameterFromUrl("url", rc.requestParam.replace('all','tx')),false,all)
                		}                    		
                	}
                },1000)        		
            ...
            $tx_audio.setFile = function(file, name, uploadFile, all, v) {
            	...
            	$tx_audio.attr("src", file.replace('all','rtx')+"&recVolume="+v);
                ...
                if(rc.type[TYPES.WAVE]){
            		rc.initWave();
            		rc.updateWave(getParameterFromUrl("url", rc.requestParam.replace('all','rtx')),true,all)
            	}
            }
        ...
        */
        ...
        rc.addPlayList = function(recFileData,str){
        	...
        	if(top.telnoUse!='Y'){
        		htmlString += "<span><b>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text6/*상담사명*/+"</b> : "+recUserName+"</span>";
        	}
        	
        	htmlString += "<span><b>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text7/*고객번호*/+"</b> : "+recCustPhone+"</span>";
        	if(top.telnoUse!='Y'){
        		htmlString += "<span><b>"+top.$("#pageFrame")[0].contentWindow.lang.views.player.html.text8/*내선번호*/+"</b> : "+recExt+"</span>";
        	}
        	htmlString += "</li>";
...
//    					rc.player.find('.main_btn_filedownload').click();
	...
	if(top.unqLang=="jp") recsee_player_ui_html = "recsee_player_jp.ui.html";
