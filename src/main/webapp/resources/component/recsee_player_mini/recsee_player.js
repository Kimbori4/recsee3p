...
RecseePlayer.prototype._RC = (function() {
    ...
    */
    ...
    _RC.DEFAULT_OPTION.type[_RC.TYPE.TOOL] = false;
    _RC.DEFAULT_OPTION.type[_RC.TYPE.LIST] = true;
    _RC.DEFAULT_OPTION.type[_RC.TYPE.MEMO] = false;
    _RC.DEFAULT_OPTION.type[_RC.TYPE.REPLAY] = false;

    _RC.DEFAULT_OPTION.type[_RC.TYPE.BTNDOWNFILE] = false;
    _RC.DEFAULT_OPTION.type[_RC.TYPE.BTNUPFILE] = false;

    _RC.DEFAULT_OPTION.type[_RC.TYPE.BTNPLAYSECTION] = false;
    _RC.DEFAULT_OPTION.type[_RC.TYPE.BTNTIMESECTION] = false;
    _RC.DEFAULT_OPTION.type[_RC.TYPE.BTNMOUSESECTION] = false;
    _RC.DEFAULT_OPTION.type[_RC.TYPE.BTNMEMODEL] = false;
...
RecseePlayer.prototype.setDefault = function() {
    ...
    */
    ...
    /**
     * @define RecseePlayer.FileListen
     * @type function
     * @description RecseePlayer 콜아이디로 파일 재생함수
     * @param f(string : 콜아이디)
     */
     this.FileListen = undefined;
	            ...
	            '<div class="ui_pannel_row">'+
	                '<div class="ui_padding">'+
	                	'<label class="ui_label_essential">시작 시간</label>'+
	                    	'<input class="input_time inputFilter numberFilter timeFilter startTime" id="" value="" type="text" maxlength="8"/>'+
	                	'<label class="ui_label_essential">종료 시간</label>'+
	                    	'<input class="input_time inputFilter numberFilter timeFilter endTime" id="" value="" type="text" maxlength="8"/>'+
	                '</div>'+
	            '</div>'+
	        '</div>'+
	        '<div class="ui_article">'+
    ...
    this.setEvent = function(e) {
            ...
            $audio.setFile = function(file, name, uploadFile, all) {
                ...
                if(rc.type[TYPES.DUAL]){
    	            ...
    	            if(rc.autoplay){
    	            	...
    	            	if(window.location.pathname.indexOf('listenSimple')>-1&&isChrome){
    	            	}else{
    	            		syncDo("play")
    	            	}
    	            }
        ...
        */
        ...
        rc.addPlayList = function(recFileData,str){
        	//console.log(recFileData);
        	var listenUrl = recFileData.listenUrl
        ...
        */
         ...
         * @define RecseePlayer.FileListen
         * @type function
         * @description 콜아이디로 재생
         * @return
         */
        rc.FileListen = function(callId){
        	var result;
        	var http = new XMLHttpRequest();

        	http.open("GET","http://"+rc.requestIp+":28881"+"/callKey?callKey="+callId, false );
    		http.timeout = 2000;
    		...
    		result = JSON.parse(result).data;
    		 $(rc.target).find(".list_wrap ul").html("");
    		if(result.length==1){
    			if($player.find('.play_list_popup').css("display") != "none"){
    				$player.find('.play_list_popup').slideToggle();
    			}
    			rc.requestIp = result[0].r_v_rec_ip;
    			rc.setFile("audio", "?url="+result[0].r_v_rec_fullpath);
    		}else if(result.length > 1){
    			for(var i = 0; i < result.length; i++){
    				result[i].listenUrl = "http://"+ result[i].r_v_rec_ip +":28881/listen?url="+ result[i].r_v_rec_fullpath;
    				result[i].recDate = result[i].r_rec_date;
    				result[i].recExt = result[i].r_ext_num;
    				result[i].recCustPhone = result[i].r_cust_phone1;
    				result[i].recUserName = result[i].r_user_name;
    				rc.addPlayList(result[i],"Y");
    			}
    		}else{
    			console.log("요청하신 파일이 없습니다.");
    		}
        }
        ...
        rc.makeSection = function(startTime, endTime, startP, widthP, memoInfo) {
    		...
    		nowFinishMarkingObj.find(".downMarking").click(function(){
    			setTimeout(function(){
    				...
    				try{
                		if(parent.frames['pageFrame'].window.encYn == "Y" && parent.frames['pageFrame'].window.encNCheck == false){
	            	...
	            	if (rc.all){
	            		src = "http://"+parseUri( rc.requestParam ).authority+"/down/?"
	            	}
		...
		function sendFileToServer(formData,oriname){
			...
			var src = "http://"+rc.requestIp+":"+rc.requestPort
        	if (rc.all){
        		src = "http://"+parseUri( rc.requestParam ).authority
        	}
...
//            	console.log(event)
            ...
            target.addEventListener('loadstart',function(event) {
                ...
                var infoString = (rc.recFileData && rc.recFileData.recExt ? " / 내선번호 : " + rc.recFileData.recExt : "")
                	infoString += (rc.recFileData && rc.recFileData.recUserName ? " / 상담원 이름 : " + rc.recFileData.recUserName : "")
                	infoString += (rc.recFileData && rc.recFileData.recCustPhone ? " / 고객번호 : " + rc.recFileData.recCustPhone : "")
...
//    			},1);
                ...
                if($player.find(".list_wrap ul li").length > 0){
                	...
                	if(nowPlayUrl == audioObj.attr("src") && audioObj.attr("src")){
                		...
                		if(nextPlayObj.length == 0){
                    	    ...
                    	    rc.setFile("audio", nextPlayUrl, undefined, true);
                		}
                		return false;
            ...
            $player.find('.main_btn_delete').on("click", function() {
            	...
            	if(confirm("현재 파일에 저장된 메모가 모두 삭제 됩니다.\정말 삭제 하시겠습니까?")){
            ...
            $player.find(".fileUpload").on("change", function(){
            	...
            	var url = "http://"+rc.requestIp+":"+rc.requestPort+"/upload"
            	if (rc.all){
            		url = "http://"+parseUri( rc.requestParam ).authority+"/upload"
            	}
    ...
    this.HTML = (function() {
        ...
        $.ajax({
            ...
            success: function(res) {
                html = res;
                html = $(html).find(".play_list").find(".play_info").find("span").text("Play File");
    ...
    if($(".play_list").find(".play_info").find("span").length > 0){
    	console.log("123")
    	$(".play_list").find(".play_info").find("span").html("123123"/*"현재 재생 중인 파일 : "*/);
    }
}
...
$(document).on("keyup", ".inputFilter", function(e){
	...
	if(oriObj.hasClass("dateFilter")){
		...
		if(date.length <= 4){
			...
			var settingVal ="";
			settingVal += date.substr(0, 4);
			settingVal += '-';
			settingVal += date.substr(4, 2);
			obj[0].value = settingVal
