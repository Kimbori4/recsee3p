...
function layer_popup(name, resizeable, resizeableOpt){
    ...
    var popMargTop = 100;
    if($(popupObj).height() > $(window).height())
    	popMargTop = ($(window).height() + 24) / 2; 
    else
    	popMargTop = ($(popupObj).height()) / 2 + 24;
    ...
    if(resizeable){
    	...
    	$(name).resizable(resizeableOpt);
    	if(name='#csvPopup'){
            $(name).resize(function() {
               $("#csvGrid").css("height", ($('#csvPopup').css('height').replace('px','')-140)+"px");
               $("#csvGrid").css("width", ($('#csvPaging').css('width').replace('px',''))+"px");
               csvGrid.setSizes();
            })
         }
    }
...
function layer_popup_close(name){
    ...
    // 닫을 때 내용 초기화
/*    $(targetSelector).find('input[type!="radio"]').val("");
    $(targetSelector).find('select').prop("selectedIndex",0);*/
...
$(function() {
	...
	$(document).on("click", ".popup_close", function(e){
		$closeSelf = $(this).parents(".popup_obj");
		if($closeSelf.attr('id')=='recTag'){
			$("#timeTag").val('')
		}
		$closeSelf.fadeOut(300 , function() {
	        // 활성화된 팝업 갯수가 1개 미만이면 마스크 지우기
	        if($(".popup_obj:visible").length < 1) {
	            $('#mask').remove();
	        }
