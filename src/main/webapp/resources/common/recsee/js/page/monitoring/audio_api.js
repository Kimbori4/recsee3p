...
//isFirefox = (Agent.match('like gecko') ? false : (ua.match(/(gecko|fennec|firefox)/g) ? true : false));
...
function OnControlsPlay(){
   ...
   try{
	   if(HTTP!='https')
		   ...
		   SocketClient = new WebSocketCallBack('wss://' + ServerIPAddr + ':'+ (SelectedPORT+1), OnSocketError, OnSocketConnect, OnSocketDataReady, OnSocketDisconnect);	 
   }
   catch (e){
      return;
   }
...
//					realtime_rc.target.find(".procTime").text(secondToMinute(++sec))
				realtime_rc.target.find(".procTime").text(timeDiff(defineStime))
			}else{
				clearInterval( timer )
				sec = 0
...
function OnSocketDisconnect(){
   ...
   try {
      ...
      /*.then(function() {
	   AudioPlayer.SoundContext.close();
    	  AudioPlayer = null;
    	  FormatReader = null;
      });*/
      //@mars
