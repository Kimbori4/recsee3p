...
// rc_player.setFile("audio", '1');
	...
	// 파일이름 변경 관련 세팅
		...
		$(".fileSetting").hide();
		$("#fileNameSetting").hide();
	}
	
	// 렉시 플레이어 언어팩 처리 --html로 되어 있어서 <spring:message code=""/>가 안먹힘...
	// iframe로 나누어져 있어 바로 접근은 불가하여 부모 top 접근하여 player프레임 접근하는 식으로 처리
// top.$('#playerFrame').contents().find(".play_list").find(".play_info").find("span").html(lang.views.player.html.text2/*"현재
// 재생 중인 파일 : "*/);
// top.$('#playerFrame').contents().find(".play_list").find(".play_list_right").find(".play_jump").find("button").html(lang.views.player.html.text3/*"이동"*/);
	
	
// $("#buffer4").append("<option value=''>"+lang.combo.ALL+"</option>");
...
function play(gridObj, id){
	...
	var listenUrl 		= (gridObj.getColIndexById("rec_url")==undefined ? 		"" : gridObj.cells(id,gridObj.getColIndexById("rec_url")).getValue().replace(':8088',':28881/listen?url='));
	...
	var recCustPhone	= (gridObj.getColIndexById("r_cust_phone1")==undefined?	"" : gridObj.cells(id,gridObj.getColIndexById("r_cust_phone1")).getValue());
	...
	var recvFileName    = (gridObj.getColIndexById("r_v_filename")==undefined?	"" : gridObj.cells(id,gridObj.getColIndexById("r_v_filename")).getValue());
	var recMemo 		= (gridObj.getColIndexById("r_memo_info")==undefined?   "" : gridObj.cells(id,gridObj.getColIndexById("r_memo_info")).getValue());
	...
	var recVisible      = (gridObj.getColIndexById("r_rec_visible")==undefined?   "" : gridObj.cells(id,gridObj.getColIndexById("r_rec_visible")).getValue());
	var recVolume       = (gridObj.getColIndexById("r_rec_volume_value")==undefined?   "" : gridObj.cells(id,gridObj.getColIndexById("r_rec_volume_value")).getValue());
	var vSysCode	    = (gridObj.getColIndexById("r_v_sys_code")==undefined ? 	"" : gridObj.cells(id,gridObj.getColIndexById("r_v_sys_code")).getValue())
    var waveType      = "rsfft"; // default wave type
	var sysCode 	  = (gridObj.getColIndexById("r_v_sys_code") ==undefined ? "" : gridObj.cells(id,gridObj.getColIndexById("r_v_sys_code")).getValue()); 
	var recStartType  = (gridObj.getColIndexById("r_rec_start_type")==undefined?   "" : gridObj.cells(id,gridObj.getColIndexById("r_rec_start_type")).getValue());
    ...
    if(recStartType.length > 1){
        recStartType = recStartType.substring(recStartType.length-1);
    }
	
    if (recVisible == "D") {
       alert(lang.views.search.alert.msg30 /* "삭제된 파일입니다." */);
       return false;
    }
	// 녹취 파일 정보
	var recFileData = {
	...
	if(rc.type.dual){
		 ...
		 */
		$.ajax({
			url:contextPath+"/dualModeListen.do",
			data:recFileData,
			type:"POST",
			dataType:"json",
			async: false,
...
function openFaceRecordingPlayer(from) {
	...
	var callId1 = gridSearch.getColIndexById("r_call_id1")==undefined ? 	"" : gridSearch.cells(id,gridSearch.getColIndexById("r_call_id1")).getValue();
	
	gridCallKey = callId1;
	
	if (from == "gridSearch") {
		listenPopupGrid = createGrid("listen_popup_grid","","listenPopupGrid","?callId1="+callId1,"",20,[],[]);
...
//		}
	...
	checkSoundDevice(gridSearch,id);
}
...
function scriptPdfDown(){
	window.open(contextPath+"/wooribank/script/download/product/"+gridCallKey+"/script/call?fileType=pdf") 
}