...
<head>
	...
	<c:set var="contextPath" value="<c:out value="${pageContext.request.contextPath}"/>"/>
	<c:set var="resourcePath" value="<c:out value="${contextPath}"/>/resources"/>
	<c:set var="compoResourcePath" value ="<c:out value="${resourcePath}"/>/component"/>
	<script type="text/javascript" src="<c:out value="${compoResourcePath}"/>/jquery/jquery-2.1.3.min.js"></script>
	<script type="text/javascript" src="<c:out value="${compoResourcePath}"/>/recsee_player/recsee_transcript_player.js?ver=1234"></script>
	...
	/*
	var playerPassword = "${playerPassword}";
	var playerPeriod = "${playerPeriod}"; */
    ...
    $(function() {
    	...
    	rc_player = new RecseePlayer({
	    	...
	    	,	"btnDownPlayer" 	: false		// 플레이어 다운로드 버튼 사용 여부  
			...
			,	"userTypeMenu"		: false				// 유저 타입 메뉴 설정
			,	"playListSave"		: false				// 플레이리스트 저장 여부 
			,	"autoplay"			: true				// 자동재생 여부 
			,	"barCount"			: 200
			,	"requestIp"			: $("#ip").val()	// 통신 IP
			,	"requestPort"		: $("#port").val()	// 통신 Port
			,	"HTTP"				: http	// http, htttps 유무
    	});
    	...
    	dual_rc_player = new RecseePlayer({
			...
			,	"requestIp"			: $("#ip").val()	// 통신 IP
			,	"requestPort"		: $("#port").val()	// 통신 Port
			,	"HTTP"				: http	// http, https 유무
		});
    	
    	$("#dualPlayerObj").hide();
    	$(".play_list_right").hide();
    ...
    function playerToggle(){
		...
		dualSetting(!$("#playerObj").is(":visible"));
